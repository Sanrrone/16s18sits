function humanDecont {
	...
	do
		...
		head -n $(echo $nfiles |awk -v read4learn=$READS4LEARN '{print int(4*read4learn/$1/2)}') ${samplename}_decont.1 > ${samplename}_subR1.fastq
		head -n $(echo $nfiles |awk -v read4learn=$READS4LEARN '{print int(4*read4learn/$1/2)}') ${samplename}_decont.2 > ${samplename}_subR2.fastq
		##compressing
